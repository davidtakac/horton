...
pub struct SignatureGenerator {
    ...
    ring_buffer_of_samples: Vec<i16>,// Ring buffer.
    ring_buffer_of_samples_index: usize,
    
    reordered_ring_buffer_of_samples: Vec<f32>, // Reordered, temporary version of the ring buffer above, with floats for precision because we applied Hanning window.
    
    fft_outputs: Vec<Vec<f32>>, // Ring buffer. Lists of 1025 floats, premultiplied with a Hanning function before being passed through FFT, computed from the ring buffer every new 128 samples
    fft_outputs_index: usize,
    
    fft_object: RFft1D::<f32>,
    
    spread_fft_outputs: Vec<Vec<f32>> , // Ring buffer.
